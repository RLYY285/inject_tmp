...
4. 可选对原可恢复段执行“字节级裁剪”或“页级裁剪”做物理删除；无法安全删除时回退到明文覆写（`0xA5`）。
...
6. 若启用裁剪参数则优先做物理删除（字节级/页级）；否则覆写可恢复段原始文件字节去除明文映像（可通过参数关闭）。
...
- `--prune-recoverable-bytes`：启用非页粒度裁剪（精确删除可恢复段字节）；不可安全裁剪时回退。
- `--prune-recoverable-pages`：启用页级裁剪（按 `0x1000` 页重排）；无法安全裁剪时回退为明文覆写。
- `--status-json`：将处理状态输出为 JSON（含每个文件的统计信息）。
...
1. 开启 `--prune-recoverable-bytes`：先尝试字节级裁剪。
2. 若字节级不可行且开启 `--prune-recoverable-pages`：再尝试页级裁剪。
3. 裁剪仍不可行：回退为明文覆写（`0xA5`）。
...
`status.json` 的 `results[]` 中包含：

- `success` / `error`：该文件处理结果。
- `total_blocks`：总污染块数（头部 + 可恢复段）。
- `obfuscation_inserted_bytes_excluding_stub`：插入混淆字节总数（不含 stub）。
- `obfuscation_inserted_bytes_header`：头部污染插入字节数。
- `obfuscation_inserted_bytes_recoverable`：可恢复段污染插入字节数。
- `plaintext_handling_mode` 与 `plaintext_changed_bytes`：去明文方式及改动字节数。
...
- `明文覆写: 覆写可恢复段原始字节 = N 字节`：
  已去除原可恢复段的文件明文数据。
- `字节级裁剪: 删除可恢复段精确字节 = N 字节`：
  已按非页粒度物理移除可恢复段字节，并重写相关偏移。
...
4. 字节级/页级裁剪都需要满足装载对齐与段重叠约束；实现会自动做安全检查，不满足时按优先级回退。
